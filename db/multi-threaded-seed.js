...
// Database Name
const dbName = 'sagat';
let processIdStorage = {};

function multiThreadedSeedDb() {
  if (cluster.isMaster) {
    ...
    // handles worker job completion behavior
    cluster.on('exit', (worker, code, signal) => {
      console.log(`Worker ${worker.process.pid} finished`);
      clustersComplete ++;
      // // if all workers are finished, create index and close
      // if (clustersComplete === numCPUs) {
      //   MongoClient.connect(url, function(err, client) {
      //     if (err) {
      //       console.log('Mongo connect error in master cluster');
      //     } else {
      //       console.log('Creating index');
      //       client.db(dbName).collection('restaurants').createIndex({place_id: 1}, () => {
    ...
    workerSeed(cluster.worker.id);
  }
}

// Use connect method to connect to the server
function workerSeed(workerNumber) {
  MongoClient.connect(url, function(err, client) {
    console.log("Connected successfully to mongo, inserting entries");
    const db = client.db(dbName);
    let insertsLeft = parseInt(targetDatabaseSize / numCPUs);
    let batchCounter = 0;
    while (insertsLeft > 0) {
      insertIntoDatabase(workerNumber, db, batchCounter, insertsLeft, () => {
        batchCounter++;
        console.log('insertsLeft = ', insertsLeft);
        insertsLeft = insertsLeft - batchSize;
        if (batchCounter % 20 === 0) {
          console.log(`Worker #${workerNumber} inserted batch ${batchCounter} in ${(new Date().getTime() - startTime) / 1000} seconds, ${targetDatabaseSize/(numCPUs * batchSize) - batchCounter} batch[es] to go`);
        }
      });
      if (insertsLeft <= 0) {
        console.log(`Worker #${workerNumber} inserted ${batchCounter} batches, of batch size ${batchSize}, in ${(new Date().getTime() - startTime) / 1000 / 60} minutes`);
        console.log("Now closing worker's client");
        client.close(()=> {
          console.log('client closed');
          process.exit();
        });
...
function insertIntoDatabase (workerNumber, db, insertionCounter, insertsLeft, callback) {
  const collection = db.collection('restaurants');
  // Create a batch of restaurants in arrRestaurants
    // workerBase = (workerID - 1) * insertsPerWorker  /*starting number for each worker*/
    // counterbase = insertionCounter * batchSize /*how many inserts from previous batches from this worker*/
    // each restaurant's uniqueID derived from workerBase + counterBase + i.
  let insertsPerWorker = parseInt(targetDatabaseSize / numCPUs);
  let workerBase = (workerNumber - 1) * insertsPerWorker;
  let counterBase = workerBase + (insertionCounter * batchSize);
  var arrRestaurants = restaurantCreator(counterBase, insertsLeft, targetDatabaseSize);
  // console.log('arrRestaurants[0] = ', arrRestaurants[0]);
  // Prepare array of function calls
  var arrInsertOps = [];
  arrRestaurants.forEach((restaurant => {
    arrInsertOps.push({ "insertOne": { "document": restaurant } });
  }));
  console.log('arrInsertOps.length = ', arrInsertOps.length);

  collection.bulkWrite(arrInsertOps, {ordered: false}).then(callback());
  // var arrInsertOps = arrRestaurants.map(restaurant => ({
  //    insertOne: {"document": restaurant}
  // }));
  // // Insert prepared arrRestaurants
  // console.log('before bulkwrite');
  // collection.bulkWrite(arrInsertOps, {ordered: false});
  // console.log('after bulkwrite');
  // callback();
}

const restaurantCreator = (counterBase, insertsLeft) => {
  var arrRestaurants = [];
  let workOrder = 0;
  // if there are fewer needed inserts than the standard batch size, use the insertsLeft
  if (batchSize <= insertsLeft) {
    workOrder = batchSize;
  } else {
    workOrder = insertsLeft;
  }
  for (var i = 1; i <= workOrder; i++) {
    ...
    var randomNearby = randomNearbyGenerator(targetDatabaseSize);
