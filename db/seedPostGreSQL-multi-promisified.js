...
let restaurantWorkerCompleted = 0;
...
const db = pgp('postgres://localhost:5432/sagat_sql2'); // Database object
...
// Helper functions to generate an arrays of objects to feed into batchCreator
...
let trackWorkersDoneWithTable1 = (singleWorker) => {
...
function seedRestaurantsMultiWrapper() {
  if (cluster.isMaster) {
    console.log(`Master ${process.pid} is running in seedRestaunts`);
    // Storage to track when workers are done
    let workersDoneTable1 = [];
    // Fork workers.
    for (let i = 0; i < numCPUs; i++) {
      ...
      console.log(`seedRestaurants started worker ${worker.process.pid}`);
      workersDoneTable1.push(trackWorkersDoneWithTable1(worker));
    }
    Promise.all(workersDoneTable1).then(() => {
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      // restaurantWorkerCompleted++;
      // if (restaurantWorkerCompleted === numCPUs) {
      //   console.log('want to run seedNearbyMultiWrapper');
      //   seedNearbyMultiWrapper();
      // }
    });
  } else {
    console.log(`Worker ${process.pid} started running seedTableRestaurants`);
    // console.log('seedTableRestaurants - ', seedTableRestaurants())
...
const seedNearbyMultiWrapper = () => {
  if (cluster.isMaster) {
    console.log(`Master ${process.pid} is running in seedNearby`);
    // Fork workers.
    for (let i = 0; i < numCPUs; i++) {
      let worker = cluster.fork();
      console.log(`seedNearby started worker ${worker.process.pid}`);
    }
    cluster.on('exit', (worker, code, signal) => {
    ...
    // Seed first
    console.log(`Worker ${process.pid} started running seedTableNearby`);
    seedTableNearby();
  }
}
...
//   console.log('after seedRestaurantsMultiWrapper');
//   seedNearbyMultiWrapper();
// }
// seedTwoTables();
